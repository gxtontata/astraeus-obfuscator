...
local function chartoascii(str, pos)
	...
	Deserializer = [=[
function BcToState(Bytecode, charset, xorkey)
    ...
    -- XOR decode bytes with embedded key
    if xorkey and xorkey > 0 then
        for i = 1, #bytes do
            local a, b = bytes[i], xorkey
            local result, p = 0, 1
            while a > 0 or b > 0 do
                local ab, bb = a % 2, b % 2
...
function LuaFunc(State, Env, n)
    ...
    local X = State.X;
    local z = State.z;
    local __op_remap = REMAP_TABLE
    while alpha do
        local Inst = x[z]
        local S = Inst.S;
        if __op_remap then S = __op_remap[S] end
        local C = Inst.C;
        local A = Inst.A;
        local B = Inst.B;
        local D = Inst.D;
