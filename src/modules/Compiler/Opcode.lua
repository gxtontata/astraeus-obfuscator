function GetOpcodeCode(S)
	...
	elseif (S == 38) then
		-- LOADK_MOVE: combines LOADK (1) + MOVE (0)
		-- Loads a constant into A and copies register B to C in one step
		return [=[
        X[Inst.A] = (type(Inst.D) == "number" and Inst.D % 1 == 0) and math.floor(Inst.D) or Inst.D
        X[Inst.C] = X[Inst.B]
        ]=]
	elseif (S == 39) then
		-- ADD_SETTABLE: combines ADD (12) + SETTABLE (9)
		-- Adds two values and stores the result into a table
		return [=[
        local Lhs, Rhs;
        if Inst.s then
            Lhs = Inst.L
        ...
        local sum = NormalizeNumber(Lhs + Rhs)
        X[Inst.A][X[Inst.B]] = sum
        ]=]
	elseif (S == 40) then
		-- GETGLOBAL_CALL: combines GETGLOBAL (5) + CALL (28)
		-- Loads a global function and immediately calls it
		return [=[
        local A = Inst.A;
        local fn = Env[Inst.D]
        local B = Inst.B;
        ...
        local RetB = Pack(fn(Unpack(X, A + 1, A + Params)))
        local RetNum = RetB.n;
        if C == 0 then
            Top = A + RetNum - 1;
        else
