--[[
Astraeus Obfuscator manifest

This file is the single source of truth for obfuscation module metadata. The
runtime pipeline, CLI flag parser, and external consumers such as Astraeus-API
...
Tests may provide ASTRAEUS_MANIFEST_EXTRA=/path/to/extra_manifest.lua. The extra
...
manifest.output = {
    ...
    watermark_text = "--[Obfuscated by Astraeus V2.0.0]\n",
    final_print = true,
}
    ...
    languages = {
        luau = {
            patterns = {
                ...
                { pattern = "\\bgame\\s*:\\s*GetService\\s*\\(", lua_pattern = "game%s*:%s*GetService%s*%(", weight = 4, description = "Roblox service access" },
                { pattern = "\\bgame\\s*:\\s*HttpGet\\s*\\(", lua_pattern = "game%s*:%s*HttpGet%s*%(", weight = 3, description = "Roblox HTTP fetch" },
                { pattern = "\\bgame\\.Workspace\\b", lua_pattern = "game%.Workspace", weight = 2, description = "Roblox workspace access" },
                { pattern = "\\bInstance\\.new\\s*\\(", lua_pattern = "Instance%.new%s*%(", weight = 4, description = "Roblox instance creation" },
                { pattern = "\\btask\\.(spawn|wait|defer|delay)\\s*\\(", lua_patterns = { "task%.spawn%s*%(", "task%.wait%s*%(", "task%.defer%s*%(", "task%.delay%s*%(" }, weight = 2, description = "Roblox task scheduler" },
                { pattern = "\\b(UDim2|UDim|Color3|Vector3|CFrame)\\.", lua_patterns = { "UDim2%.", "UDim%.", "Color3%.", "Vector3%.", "CFrame%." }, weight = 2, description = "Roblox datatypes" },
                { pattern = "\\bEnum\\.\\w+", lua_pattern = "Enum%.", weight = 2, description = "Roblox enum" },
                { pattern = ":(WaitForChild|FindFirstChild|FindFirstChildOfClass|GetChildren|IsA|FireServer|Connect)\\s*\\(", lua_patterns = { ":WaitForChild%s*%(", ":FindFirstChild%s*%(", ":FindFirstChildOfClass%s*%(", ":GetChildren%s*%(", ":IsA%s*%(", ":FireServer%s*%(", ":Connect%s*%(" }, weight = 1, description = "Roblox instance methods" },
                { pattern = "\\b(workspace|script|LocalPlayer|ReplicatedStorage|RunService|Players|CoreGui)\\b", lua_patterns = { "%f[%w_]workspace%f[^%w_]", "%f[%w_]script%f[^%w_]", "%f[%w_]LocalPlayer%f[^%w_]", "%f[%w_]ReplicatedStorage%f[^%w_]", "%f[%w_]RunService%f[^%w_]", "%f[%w_]Players%f[^%w_]", "%f[%w_]CoreGui%f[^%w_]" }, weight = 1, description = "Roblox globals and services" },
            },
        },
        glua = {
            patterns = {
                ...
                { pattern = "\\btimer\\.\\w+|\\butil\\.\\w+|\\bnet\\.\\w+|\\bumsg\\.\\w+|\\busermessage\\.\\w+", lua_patterns = { "timer%.%w+", "util%.%w+", "net%.%w+", "umsg%.%w+", "usermessage%.%w+" }, weight = 2, description = "gmod libraries" },
...
manifest.modules = {
    ...
    {
        key = "constant_encryption",
        config_key = "constant_encryption",
        name = "Constant Encryption",
        module = "modules/constant_encryptor",
        enabled = true,
        bit_position = 13,
        pipeline_order = 15,
        cli = { short = "-ce", long = "--constant_encryption" },
        incompatible_with = {},
        description = "Enable constant encryption via XOR cipher",
    },
    {
        key = "string_splitting",
        config_key = "string_splitting",
        name = "String Splitting",
        module = "modules/string_splitter",
        enabled = true,
        bit_position = 14,
        pipeline_order = 25,
        cli = { short = "-ss", long = "--string_splitting" },
        incompatible_with = {},
        description = "Enable string splitting into fragments",
    },
    {
        key = "env_proxy",
        config_key = "env_proxy",
        name = "Environment Proxy",
        module = "modules/env_proxy",
        enabled = true,
        bit_position = 15,
        pipeline_order = 115,
        cli = { short = "-ep", long = "--env_proxy" },
        incompatible_with = {},
        description = "Enable environment proxy for global functions",
    },
}
...
local function load_extra_manifest()
    local path = os.getenv("ASTRAEUS_MANIFEST_EXTRA")
    ...
    if not chunk then
        error("Failed to load ASTRAEUS_MANIFEST_EXTRA: " .. tostring(err))
    end

    local extra = chunk()
    if type(extra) ~= "table" then
        error("ASTRAEUS_MANIFEST_EXTRA must return a table")
    end
