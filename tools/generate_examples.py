...
def load_manifest() -> dict[str, Any]:
    result = subprocess.run(
        [LUA_BIN, "astraeus.lua", "--manifest-json"],
        cwd=SRC,
        capture_output=True,
        text=True,
        check=True,
...
def write_meta_file(meta: dict[str, Any]) -> None:
    ...
    (SITE_DATA_DIR / "meta.js").write_text(
        "window.ASTRAEUS_SHOWCASE_META=" + data + ";\n",
        encoding="utf-8",
    )
...
def write_data_file(target: str, target_examples: dict[str, Any]) -> None:
    ...
    for idx, (combo_key, item) in enumerate(target_examples["items"].items()):
        ...
        (target_dir / f"{file_id}.js").write_text(
            "window.ASTRAEUS_COMBO=window.ASTRAEUS_COMBO||{};"
            f"window.ASTRAEUS_COMBO[{json.dumps(target)}]="
            f"window.ASTRAEUS_COMBO[{json.dumps(target)}]||{{}};"
            f"window.ASTRAEUS_COMBO[{json.dumps(target)}][{json.dumps(combo_key)}]="
    ...
    (target_dir / "index.js").write_text(
        "window.ASTRAEUS_SHOWCASE_DATA=window.ASTRAEUS_SHOWCASE_DATA||{};"
        f"window.ASTRAEUS_SHOWCASE_DATA[{json.dumps(target)}]={{"
...
<head>
  ...
  <title>Astraeus Obfuscation Showcase</title>
  <style>
...
<body>
  <header>
    <h1>Astraeus Obfuscation Showcase</h1>
  ...
  <script>
    ...
    const META = window.ASTRAEUS_SHOWCASE_META;
    if (!META) {
    ...
    function currentItem() {{
      const data = window.ASTRAEUS_SHOWCASE_DATA[lang];
      if (!data) return null;
      const key = comboKey(selected);
      if (!key) return null;
      return (window.ASTRAEUS_COMBO[lang] || {{}})[key] || null;
    }}
    ...
    function loadComboData(lookupKey) {{
      ...
      if (!lookupKey || lookupKey in (window.ASTRAEUS_COMBO[lang] || {{}})) return Promise.resolve();
    ...
    function renderContent() {{
      ...
      if (!item && key && !(key in (window.ASTRAEUS_COMBO[lang] || {{}}))) {{
    ...
    function renderStats(original, obfuscated, ok) {{
      ...
      $('stats').innerHTML = [
        ['Target', lang.toUpperCase()], ['Generated', ok ? 'yes' : 'no'], ['Original', original + ' B'], ['Output', obfuscated + ' B'], ['Ratio', ratio], ['Examples', Object.keys(window.ASTRAEUS_SHOWCASE_DATA[lang].items).length], ['Created', META.generated_at.replace('T',' ').replace(/\\.\\d+.*/, ' UTC')], ['Source', window.ASTRAEUS_SHOWCASE_DATA[lang].source_file]
