...
local function test_dummy_module_internal_detection()
    ...
    _G.__astraeus_dummy_module_ran = false
    local code = Pipeline.process("")
    local fn, load_err = load(code)
    assert(fn, tostring(load_err))
    fn()
    assert(_G.__astraeus_dummy_module_ran == true, "dummy module was not executed")
...
local function test_dummy_module_cli_export()
    ...
    local command = 'cd src && ASTRAEUS_MANIFEST_EXTRA="../tests/extra_manifest.lua" ' .. lua .. ' astraeus.lua --manifest-json'
